...
export const DirectorioLaboratorios = memo(({ laboratorios, alGuardarLab, alEliminarLab }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
      ...
      <form onSubmit={handleGuardarSubmit} className="bg-white dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-2xl p-6 shadow-xs space-y-3">
        ...
        <div className="border-t pt-3 space-y-2">
          <label className="block font-bold text-gray-800 dark:text-graphite-100 uppercase text-[10px]"><span className="inline-flex items-center gap-1"><Settings size={12} />Tarifario Personalizado del Laboratorio</span></label>
          
          <div className="space-y-2 bg-gray-50 dark:bg-graphite-800 p-3 rounded-xl border">
            <input
              type="text"
              ...
              className="w-full p-2 rounded-lg border bg-white dark:bg-graphite-800 font-semibold text-[11px]"
            />
          ...
          <div className="space-y-1 max-h-40 overflow-y-auto pt-1">
            {tarifasTemp.map(t => (
              <div key={t.trabajo} className="flex justify-between items-center p-2 bg-white dark:bg-graphite-800 border rounded-lg">
                <span className="font-semibold text-[11px] text-gray-800 dark:text-graphite-100 truncate max-w-[170px]">{t.trabajo}</span>
      ...
      <div className="md:col-span-2 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {laboratorios.map(lab => (
            <div key={lab.id} className="bg-white dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-2xl p-5 shadow-xs flex flex-col justify-between space-y-3">
              <div>
                <div className="flex justify-between items-start border-b pb-2">
                  <h4 className="font-black text-sm text-gray-900 dark:text-graphite-50">{lab.nombre}</h4>
                ...
                <div className="text-[11px] text-gray-600 dark:text-graphite-400 mt-2 space-y-1">
                  <p><span className="font-semibold text-gray-800 dark:text-graphite-100">Contacto:</span> {lab.contacto || 'N/I'}</p>
                  <p><span className="font-semibold text-gray-800 dark:text-graphite-100">Teléfono:</span> {lab.telefono || 'N/I'}</p>
                </div>

                <div className="mt-3 pt-2 border-t space-y-1">
                  <span className="font-bold text-[10px] text-gray-500 dark:text-graphite-400 uppercase block">Tarifas Registradas ({lab.tarifas?.length || 0}):</span>
                  {lab.tarifas && lab.tarifas.length > 0 ? (
                    <div className="space-y-1 max-h-32 overflow-y-auto">
                      {lab.tarifas.map(t => (
                        <div key={t.trabajo} className="flex justify-between text-[10px] bg-gray-50 dark:bg-graphite-800 p-1.5 rounded">
                          <span className="truncate max-w-[160px] font-medium text-gray-700 dark:text-graphite-300">{t.trabajo}</span>
                          <span className="font-bold text-gray-900 dark:text-graphite-50">${t.precio.toLocaleString('es-CL')} CLP</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-gray-400 dark:text-graphite-500 italic text-[10px]">Sin tarifas asignadas.</p>
                  )}
                </div>
              </div>
            </div>
