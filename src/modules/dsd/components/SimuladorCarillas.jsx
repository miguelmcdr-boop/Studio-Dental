    ...
    <div className="bg-white dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-2xl p-6 shadow-xs space-y-6 text-xs">
      <div className="border-b pb-2 flex justify-between items-center flex-wrap gap-2">
        <div>
          ...
          <p className="text-gray-500 dark:text-graphite-400 text-[11px]">Mediciones microestéticas e inspección de relación Ancho/Alto (Objetivo: 75% - 85%).</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-gray-600 dark:text-graphite-400 font-bold mb-1 uppercase">Ancho Central Superior (mm)</label>
          <input
            type="number"
            step="0.1"
            ...
            placeholder="Ej: 8.5"
            className="w-full px-3 py-2 border rounded-xl font-bold bg-white dark:bg-graphite-800"
          />
        </div>

        <div>
          <label className="block text-gray-600 dark:text-graphite-400 font-bold mb-1 uppercase">Alto Central Superior (mm)</label>
          <input
            type="number"
            step="0.1"
        ...
        <div>
          <label className="block text-gray-600 dark:text-graphite-400 font-bold mb-1 uppercase">Línea de la Sonrisa</label>
          <select
      ...
      <div className="p-4 bg-gray-50 dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-xl space-y-2">
        <h4 className="font-bold text-gray-800 dark:text-graphite-100 text-[11px] uppercase tracking-wider">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="p-2 bg-white dark:bg-graphite-800 rounded-lg border">
            <span className="text-[10px] text-gray-400 dark:text-graphite-500 font-bold block">Incisivo Central (1.618)</span>
            <span className="font-black text-gray-900 dark:text-graphite-50 text-sm">
              {visibilidadDorada?.estado === 'DATOS_INCOMPLETOS' ? 'N/D' : `${visibilidadDorada.centralVisible} mm`}
            </span>
          </div>
          <div className="p-2 bg-white dark:bg-graphite-800 rounded-lg border">
            <span className="text-[10px] text-gray-400 dark:text-graphite-500 font-bold block">Lateral Aparente (1.0)</span>
            <span className="font-black text-gray-900 dark:text-graphite-50 text-sm">
              {visibilidadDorada?.estado === 'DATOS_INCOMPLETOS' ? 'N/D' : `${visibilidadDorada.lateralEstimado} mm`}
            </span>
          </div>
          <div className="p-2 bg-white dark:bg-graphite-800 rounded-lg border">
            <span className="text-[10px] text-gray-400 dark:text-graphite-500 font-bold block">Canino Aparente (0.618)</span>
            <span className="font-black text-gray-900 dark:text-graphite-50 text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4">
        <div>
          <label className="block text-gray-600 dark:text-graphite-400 font-bold mb-2 uppercase">Selección de Tono Objetivo (Guía Vita / Bleach)</label>
            ...
            {GUIA_TONOS_VITA.map(t => (
              <button
                ...
                className={`p-2 rounded-xl border text-left flex items-center justify-between cursor-pointer transition-all ${
                  dsdData.tonoDeseado === t.id ? 'border-2 border-black font-bold shadow-xs' : 'bg-gray-50 dark:bg-graphite-800 border-gray-200 dark:border-graphite-700'
                }`}
              >
                <span className="text-[10px]">{t.id}</span>
                <span
                  className="w-4 h-4 rounded-full border border-gray-300 dark:border-graphite-600 inline-block"
        ...
        <div>
          <label className="block text-gray-600 dark:text-graphite-400 font-bold mb-2 uppercase">Morfología Dentaria Deseada</label>
            ...
            {FORMAS_DENTARIAS.map(f => (
              <button
                ...
                className={`w-full p-2.5 rounded-xl border text-left transition-all cursor-pointer ${
                  dsdData.formaDeseada === f.id ? 'bg-black text-white font-bold' : 'bg-gray-50 dark:bg-graphite-800 text-gray-800 dark:text-graphite-100 border-gray-200 dark:border-graphite-700 hover:bg-gray-100 dark:hover:bg-graphite-700'
                }`}
              >
                <span className="block text-xs">{f.nombre}</span>
                <span className={`text-[10px] block mt-0.5 ${dsdData.formaDeseada === f.id ? 'text-gray-300' : 'text-gray-500 dark:text-graphite-400'}`}>{f.desc}</span>
              </button>
            ))}
          </div>
        </div>
