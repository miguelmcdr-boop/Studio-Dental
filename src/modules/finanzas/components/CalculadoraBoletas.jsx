...
export const CalculadoraBoletas = memo(({ alRegistrarGastoHonorario }) => {
  ...
  return (
    <div className="bg-white dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-2xl p-6 shadow-xs space-y-6 text-xs">
      <div className="border-b pb-3 flex justify-between items-center flex-wrap gap-2">
        <div>
          <h3 className="font-bold text-sm text-gray-900 dark:text-graphite-50 uppercase tracking-wider inline-flex items-center gap-1.5">
            <Receipt size={16} />
        ...
        <div className="flex items-center gap-2 bg-gray-50 dark:bg-graphite-800 p-2 rounded-xl border">
          <span className="font-semibold text-gray-600 dark:text-graphite-400">% Retención SII:</span>
          <input
            type="number"
            step="0.01"
            value={pctRetencion}
            onChange={(e) => setPctRetencion(parseFloat(e.target.value) || 0)}
            className="w-16 p-1 border rounded bg-white dark:bg-graphite-800 text-center font-bold text-xs"
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div>
            <label className="block font-semibold text-gray-700 dark:text-graphite-300 mb-1">Modalidad de Cálculo</label>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setUsarPorcentajePrestacion(true)}
                className={`p-2.5 rounded-xl font-bold transition-all text-center ${
                  usarPorcentajePrestacion ? 'bg-black text-white shadow-xs' : 'bg-gray-100 dark:bg-graphite-800 text-gray-600 dark:text-graphite-400 hover:bg-gray-200'
                }`}
              >
          ...
          {usarPorcentajePrestacion ? (
            <div className="p-4 bg-blue-50/50 border border-blue-200 rounded-2xl space-y-3">
              <div>
                <label className="block font-semibold text-gray-700 dark:text-graphite-300 mb-1">Nombre Tratamiento / Prestación</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-gray-700 dark:text-graphite-300 mb-1">Valor Prestación ($ CLP)</label>
                  <input
                    type="number"
                    placeholder="Ej: 160000"
                    value={valorPrestacion}
                    onChange={(e) => setValorPrestacion(e.target.value)}
                    className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-graphite-600 font-bold text-sm bg-white dark:bg-graphite-800"
                  />
                </div>

                <div>
                  <label className="block font-semibold text-gray-700 dark:text-graphite-300 mb-1">% Comisión Especialista</label>
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-graphite-600 font-black text-sm bg-white dark:bg-graphite-800 text-center"
                    />
                    <span className="font-bold text-gray-700 dark:text-graphite-300">%</span>
                  </div>
                </div>
              ...
              {valPrestacionNum > 0 && (
                <div className="text-[11px] bg-white dark:bg-graphite-800 p-2.5 rounded-xl border border-blue-200 flex justify-between items-center font-semibold text-blue-900">
            ...
            <div className="p-4 bg-gray-50 dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-2xl space-y-3">
              <div>
                <label className="block font-semibold text-gray-700 dark:text-graphite-300 mb-1">Modo de Emisión</label>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setModoCalculo('liquido')}
                    className={`p-2 rounded-lg font-bold text-xs ${
                      modoCalculo === 'liquido' ? 'bg-gray-800 text-white' : 'bg-white dark:bg-graphite-800 border text-gray-600 dark:text-graphite-400'
                    }`}
                  >
                    A Pagar Líquido
                  </button>
                  <button
                    ...
                    className={`p-2 rounded-lg font-bold text-xs ${
                      modoCalculo === 'bruto' ? 'bg-gray-800 text-white' : 'bg-white dark:bg-graphite-800 border text-gray-600 dark:text-graphite-400'
                    }`}
                  >
                    Monto Bruto Boleta
                  </button>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2 border-t">
            <div>
              <label className="block font-semibold text-gray-600 dark:text-graphite-400 mb-1">Nombre Especialista</label>
              <input
                type="text"
                placeholder="Dr. Juan Pérez"
                value={nombreEspecialista}
                onChange={(e) => setNombreEspecialidad(e.target.value)}
                className="w-full p-2 rounded-lg border border-gray-300 dark:border-graphite-600"
              />
            </div>
            <div>
              <label className="block font-semibold text-gray-600 dark:text-graphite-400 mb-1">Especialidad</label>
              <input
                type="text"
        ...
        <div className="bg-gray-50 dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-2xl p-5 flex flex-col justify-between space-y-4">
          <div>
            <h4 className="font-extrabold text-xs text-gray-800 dark:text-graphite-100 uppercase tracking-wider mb-4 border-b pb-2">
              Liquidación Tributaria de la Boleta
            </h4>

            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-graphite-400 font-medium">Monto Bruto Boleta:</span>
                <span className="font-bold text-gray-900 dark:text-graphite-50 text-sm">{formatearCLP(resultado.bruto)}</span>
              </div>
