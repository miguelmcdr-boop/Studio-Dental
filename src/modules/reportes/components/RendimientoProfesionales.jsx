  ...
  return (
    <div className="bg-white dark:bg-graphite-800 border border-gray-200 dark:border-graphite-700 rounded-2xl p-5 shadow-xs text-xs space-y-3">
      <div className="flex justify-between items-center border-b pb-2">
        <h3 className="font-bold text-sm text-gray-900 dark:text-graphite-50 uppercase tracking-wider"><span className="inline-flex items-center gap-1"><CreditCard size={12} />Desglose por Medio de Pago & DTE</span></h3>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-500 dark:text-graphite-400 font-semibold">Caja & Canales</span>
          <button
      ...
      {entradasMetodos.length === 0 ? (
        <p className="text-gray-400 dark:text-graphite-500 text-center py-6">No hay transacciones registradas.</p>
      ) : (
        <div className="grid grid-cols-2 gap-3">
          {entradasMetodos.map(([metodo, monto]) => (
            <div key={metodo} className="p-3 bg-gray-50 dark:bg-graphite-800 border rounded-xl flex justify-between items-center">
              <span className="font-bold text-gray-800 dark:text-graphite-100">{metodo}:</span>
