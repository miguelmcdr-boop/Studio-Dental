...
import { Odontograma3DCanvas } from './components/Odontograma3DCanvas' 
...
export const OdontogramaModulo = memo(({
  ...
  const [vista3DActiva, setVista3DActiva] = useState(false) 
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      {/* Barra Superior con Toggle 3D & Split */}
      <div className="flex justify-between items-center mb-4 flex-wrap gap-3 print:hidden">
        <div className="flex gap-2 items-center">
          <button
            type="button"
            onClick={() => setVista3DActiva(!vista3DActiva)}
            className={`px-4 py-2 rounded-xl text-xs font-extrabold border transition-all cursor-pointer shadow-xs ${
              vista3DActiva ? 'bg-emerald-600 text-white border-emerald-700' : 'bg-gray-900 text-white border-black hover:bg-gray-800'
            }`}
          >
            {vista3DActiva ? '🖥️ Cambiar a Vista 2D HD' : '🌐 Ver en Visor 3D Interactivo'}
          </button>
        ...
        {odontogramaComparar && !vista3DActiva && (
          <button
            type="button"
          ...
          >
            🪞 {modoComparativoSplit ? 'Cerrar Vista Split' : 'Ver Antes vs. Después (Split)'}
          </button>
        )}

        {esEvolucion && (
          <span className="bg-emerald-100 text-emerald-900 text-xs font-black px-3.5 py-1.5 rounded-xl border border-emerald-300">
            🟢 Modo Evolución
          </span>
        )}
      </div>
      ...
      {/* Renderizado Condicional: Visor 3D Interactivo vs Vista 2D HD */}
      {vista3DActiva ? (
        <Odontograma3DCanvas
        ...
        <>
          {/* Paleta de Herramientas 2D */}
          <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 flex flex-wrap gap-2.5 items-center text-xs print:hidden">
            <span className="font-extrabold text-gray-600 uppercase mr-2 text-[11px]">Herramienta:</span>
