...
| F7-34 | Alinear Edge Functions con contexto multi-clínica activo | 7 | **P0** | M (2-3 d) | F7-10, F7-22, F7-31, F7-32 | **IN PROGRESS** — F7-34b audito y corrigio pacientes-purge (clinicaResult[0] sin filtro activo) y archivos-purge (filtro activo faltante). PHI removida de audit logs. error.message sanitizado. 16 tests Deno pasando. Pendiente para DONE: deploy a produccion y validacion manual multi-clinica. |
| F7-34b | Cierre definitivo de contexto multi-clínica en funciones destructivas y purge | 7 | **P0** | M (1-2 d) | F7-34 | **IN PROGRESS** — Codigo corregido (validacion server-side con clinicaId del JWT + filtro activo=eq.true). 16 tests Deno pasando: cross-clinic A/B, membresia revocada, rol no-admin, cron interno, retencion legal, PHI y errores sanitizados. Paciente fantasma confirmado purgado en produccion. Pendiente para DONE: deploy de funciones corregidas + validacion manual multi-clinica (matriz T1-T11) + decision CI E2E en F7-30. |
