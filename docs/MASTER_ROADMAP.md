...
| F7-34 | Alinear Edge Functions con contexto multi-clínica activo | 7 | **P0/P1** | M (2-3 d) | F7-10, F7-22, F7-31, F7-32 | **DONE (2026-09-24)** — 7 Edge Functions corregidas (JWT `clinica_id` de user_metadata). Stack traces removidos. PHI sanitizada en respuestas HTTP y audit_log. `r2_object_key` restaurado en INSERT (NOT NULL). 3 bugs corregidos via PR #152 y #155. Validacion manual en produccion: 11/11 tests pasaron incluyendo tenant isolation (T3, T4, T7), ciclo completo de papelera y purge destructivo sobre paciente sintetico. Staging eliminado (Opcion B) por INACTIVE. Rama lista para F7-29. |
