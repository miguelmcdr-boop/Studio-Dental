...
| F10 | Rediseño Clinical Precision v2 (DS v2 + migración módulos) | 10 | P1 | L (6-8 d) | — | **PARCIAL (2026-09-09)** — F10-A DONE (Fundación: tokens v2 + Badge + PageHeader + EmptyState + ConfirmDialog + emoji-map) + F10-B DONE (Shell: Tooth icon + Sidebar v2 con 4 secciones + contadores reales + TopBar v2 avatar-menu + CommandPalette ⌘K). Pendiente: F10-C (módulos core) + F10-D (dark-sweep) + F10-E (performance) + F10-F (cierre + push) |
